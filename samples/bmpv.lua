...
tot_time_sec = math.ceil(BMPV.getSize(test) / BMPV.getFPS(test))
tot_time_min = 0
...
while true do
	...
	Screen.debugPrint(0,114,"FPS: "..BMPV.getFPS(test),white,TOP_SCREEN)
	cur_time_sec = math.ceil(BMPV.getFrame(test) / BMPV.getFPS(test))
	cur_time_min = 0
	...
	Screen.debugPrint(0,142,"Samplerate: "..BMPV.getSrate(test),white,TOP_SCREEN)
	
	percentage = math.ceil((BMPV.getFrame(test) * 100) / BMPV.getSize(test))
