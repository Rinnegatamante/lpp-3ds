...
void RarVM::Execute(VM_PreparedProgram *Prg)
{
	...
	unsigned int StaticSize=Min((unsigned int)Prg->StaticData.Size(),(unsigned int)VM_GLOBALMEMSIZE-GlobalSize);
	if (StaticSize)
...
bool RarVM::ExecuteCode(VM_PreparedCommand *PreparedCode,int CodeSize)
{
	...
	while (1)
	{
		...
		switch(Cmd->OpCode)
		{
			...
			case VM_POPA:
				{
					const int RegCount=(int)(sizeof(R)/sizeof(R[0]));
...
VM_StandardFilters RarVM::IsStandardFilter(byte *Code,int CodeSize)
{
	...
	for (uint I=0;I<sizeof(StdList)/sizeof(StdList[0]);I++)
				...
				for (int CurChannel=0;CurChannel<Channels;CurChannel++)
				{
					...
					for (int I=CurChannel,ByteCount=0;I<DataSize;I+=Channels,ByteCount++)
					{
						...
						if ((ByteCount & 0x1f)==0)
						{
							...
							Dif[0]=0;
							for (uint J=1;J<sizeof(Dif)/sizeof(Dif[0]);J++)
							{
								if (Dif[J]<MinDif)
								{
									MinDif=Dif[J];
